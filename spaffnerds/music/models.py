...
class Show(SurrogatePK, Model):
    ...
    # song_id = Column(db.String(250), unique=False, nullable=False)
    # songs = relationship('Song',
    #                     secondary=show_associations,
    #                     backref='songs',
    #                     lazy='dynamic',
    #                     order_by=show_associations.c.order)
    # songs = relationship(
    #     'Song',
    #     secondary='show_song_link'
    # )

    _songs = db.relationship('ShowSongLink',
                             order_by='ShowSongLink.track_order',
                             collection_class=ordering_list('track_order',
                                                        count_from=1))

    songs = association_proxy('_songs', 'song_'  # ,
                              # creator=lambda k,v: ShowSongLink(track_order=k, song_notes=v)
                              )
...
class Song(SurrogatePK, Model):
    ...
    def __init__(self, name, **kwargs):
        """Create instance."""
        if name:
            self.system_name = name.lower().replace(' ', '-')
        db.Model.__init__(self, name=name, **kwargs)
...
class ShowSongLink(SurrogatePK, Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    show_id = Column(db.Integer, db.ForeignKey('shows.id'), primary_key=True)
    song_id = Column(db.Integer, db.ForeignKey('songs.id'), primary_key=True)
    ...
    song_ = relationship("Song")

    song_sys_name = association_proxy('song_', 'system_name')
    song_name = association_proxy('song_', 'name')
